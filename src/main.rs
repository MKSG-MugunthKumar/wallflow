...
#[command(long_about = "wallflow brings fluidity to your desktop with beautiful wallpaper transitions, dynamic color schemes, and seamless desktop integration.")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
fn show_config(config: &Config) -> Result<()> {
    ...
    match &config.transition.transition_type {
        config::TransitionType::Single(t) => println!("  Type: {} ({}s)", t, config.transition.duration),
    ...
    println!("  Terminal colors: {}", if config.integration.desktop.update_terminal_colors { "enabled" } else { "disabled" });
