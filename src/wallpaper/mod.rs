...
use backends::{BackendRegistry, traits::{WallpaperOptions, WallpaperScaling, MonitorSelection}};
...
pub async fn apply_wallpaper(wallpaper_path: &Path, config: &Config) -> Result<()> {
    ...
    let backend = registry.get_best_backend()
        .context("No wallpaper backends available")?;

    let options = build_wallpaper_options(config);

    debug!("Applying wallpaper with {} backend", backend.name());
    debug!("Options: transition={:?}, scaling={:?}",
           options.transition, options.scaling);
...
pub fn platform_info() -> Result<String> {
    use crate::platform::{detect_platform, check_platform_dependencies};
