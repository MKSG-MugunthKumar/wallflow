...
use std::sync::Arc;
use traits::{WallpaperBackend, WallpaperOptions};
...
impl BackendRegistry {
    ...
    pub fn get_best_backend(&self) -> Result<Arc<dyn WallpaperBackend + Send + Sync>> {
        if self.backends.is_empty() {
            return Err(anyhow!("No wallpaper backends available. Please install a wallpaper setter like feh, swww, or awww"));
        }
    ...
    pub fn list_backends(&self) -> Vec<String> {
        let mut backends: Vec<_> = self.backends
            .iter()
            .map(|b| format!("{} (priority: {}, available: {})",
                             b.name(),
                             b.priority(),
                             b.is_available()))
            .collect();
